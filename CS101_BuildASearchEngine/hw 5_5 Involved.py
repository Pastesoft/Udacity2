...
def involved(courses, person):
    ...
    for when in courses: #looping all the periods
        crsFound = []
        crs = courses[when]
        for acourse in crs: #looping all the courses in one period
            cData = crs[acourse]
            for data in cData: #looping all the data related to one course
                if cData[data] == person:
                    crsFound.append(acourse)
                    break
