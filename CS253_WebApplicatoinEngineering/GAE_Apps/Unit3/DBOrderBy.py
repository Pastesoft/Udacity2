...
def query():
    ids = []
    c = db.execute("select * from links where links.submitter_id = 62443 order by submitted_time asc")
    for link_tuple in c:
        link = Link(*link_tuple)
        ids.append(link.id)
    return ids

# improved version (fetch only the id
def query2():
    c = db.execute("select id from links where links.submitter_id = 62443 order by submitted_time asc")
    ids = [t[0] for t in c]
    return ids
